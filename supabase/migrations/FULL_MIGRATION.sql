...
create index if not exists idx_hero_announcements_active_sort on public.hero_announcements(is_active, sort_order);
create index if not exists idx_hero_announcements_created_at on public.hero_announcements(created_at desc);
...
-- ============================================================
-- 013: deal slug (SEO /firsat/{slug}) — ayrıca migrations/013_deal_slug.sql
-- ============================================================
...
begin
  ...
  s := lower(trim(raw));
  s := translate(s, 'ğüşöçıİĞÜŞÖÇ', 'gusocigusoc');
