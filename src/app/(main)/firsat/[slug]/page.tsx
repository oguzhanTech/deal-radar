...
import { DealDetailContent } from "../../deal/[id]/deal-detail-content";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    const { data: deal } = await supabase
      .from("deals")
      .select("title, description, discount_percent, category, provider, image_url, end_at, slug")
      .eq("slug", slug)
      .single();
    if (!deal) return { title: "Fırsat bulunamadı" };

    const canonical = dealPath({ slug: deal.slug });
    const desc =
...
export default async function FirsatPage({ params }: PageProps) {
  const { slug: rawSlug } = await params;
  const slug = decodeURIComponent(rawSlug);
  ...
  try {
    ...
    const { data: deal } = await supabase.from("deals").select("*").eq("slug", slug).single();
    if (!deal) notFound();

    const id = deal.id as string;

    const [{ data: commentsRaw }, { count: saveCount }, { data: voteData }, { data: trendingTop }] = await Promise.all([
      supabase
        .from("deal_comments")
        .select("*")
        .eq("deal_id", id)
        .order("created_at", { ascending: true }),
      supabase.from("deal_saves").select("*", { count: "exact", head: true }).eq("deal_id", id),
      supabase.from("deal_votes").select("vote").eq("deal_id", id),
      supabase
        .from("deals")
        .select("id")
        .eq("status", "approved")
    ...
    const [{ data: creatorProfile }, { data: similarDeals }] = await Promise.all([
      supabase.from("profiles").select("display_name").eq("user_id", deal.created_by).maybeSingle(),
      supabase
        .from("deals")
        .select("*")
        .eq("status", "approved")
