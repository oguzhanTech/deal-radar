...
- **Redirect URL’ler (doğrulama maili için kritik):** `Authentication → URL Configuration` altında **Redirect URLs** listesinde, kayıt sonrası doğrulama linkinin ineceği adres **aynen** tanımlı olmalı. Kod `emailRedirectTo` olarak `…/auth/confirm` kullanır (önce `NEXT_PUBLIC_APP_URL`, yoksa tarayıcı `origin`). Örnek: `https://www.topla.online/auth/confirm` ve gerekirse `https://topla.online/auth/confirm` (www / apex). **Google OAuth** için `…/auth/callback` adresleri de listede kalmalı; Android App Links yalnızca `/auth/callback` için tanımlıdır — e-posta önce `/auth/confirm` ile tarayıcıda açılır, masaüstünde web tamamlama `/auth/complete-web` üzerinden yapılır.
- **Şifre sıfırlama akışı:** login ekranındaki `Şifremi unuttum` bağlantısı Supabase `resetPasswordForEmail` çağırır. Reset e-postası callback’e `next=/auth/reset-password` ile düşer; bu yüzden redirect listesinde `…/auth/confirm` ve callback tarafında kullanılan `…/auth/callback` URL’lerinin doğru hostla (www / apex) tanımlı olması gerekir.
